  ...
  <div class="modal-dialog">
    <form method="POST" action="manage_learners.php" enctype="multipart/form-data" class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
          <label for="last_name_<?= $learner['learner_id'] ?>" class="form-label">Last Name</label>
        ...
        <div class="mb-3">
          <label for="parent_id_<?= $learner['learner_id'] ?>" class="form-label">Parent</label>
          <select class="form-select" id="parent_id_<?= $learner['learner_id'] ?>" name="parent_id" required>
            <option value="">Select Parent</option>
            <?php foreach ($parents as $parent): ?>
              <option value="<?= $parent['parent_id'] ?>" <?= ($parent['parent_id'] == $learner['parent_id']) ? 'selected' : '' ?>>
